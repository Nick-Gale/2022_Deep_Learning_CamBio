...
* Once a threshold is reach $v$ is reset to a baseline

# Numerical Implementation:

```{julia}
using Plots
dt = 0.01; T = collect(0:dt:1); iT = 0.25 .< T .< 0.75; g = 1; thresh = 2; reset = -1
v = similar(T)
for t in 2:length(T)
	dv = g I(t) 
	v[t] = v[t-1] + dt * dv
...
plot(T, [v iT]; title="Integrate and Fire", label=["Membrane Voltage" "Current"])
```
# Leaky Integrate and Fire

* A slightly more sophisticated model recognises that a neurone tends to "rest" at a baseline.
...
where the functions $\alpha_j, \beta_j$ take the generic form:
...
* Hybrid models such as the Moris-Lecar reduce the biophysical HH type models to blend efficiency and accuracy.
...
* This is the cheapest model to implement: for a given $dt$ spike if a random number p satisfies $rdt<p$.

* The inhomogenous process allows $ r = r(t) $ if the rate changes slowly.

* This captures most statistical properities of a real neuron.

* A cheap way to generate spike data.
...
* ramp(x): a computationally cheap approximation.
...
* The neurons state is evolved as $u_i^{t+1} = sign(\sum_j W{ij} u_j^t)$ 

* The sign function classifies neurons as active/inactive and the state is evolved until it is steady.

* The steady state can be compared to a hashing function classify.
...
* This ensures that the steady state of the pattern v is v itself.

* To train M such patterns we simply take the linear addition and hash the patterns for classification.
...
* The routine for training a regressor $\x_t$ with ouput $\xi_t$ and target output $y_t$ is:
...
function np()
    p = plot(; xlim=(0,2.3), ylim=(0,2.3), title="Animals")
...
```{julia}
using Plots
plot([0 0],[1 0],[1 1], [0 1]; title="XOR", label=["True" "False"], c = [:orange, :blue, :orange, :blue])
```
