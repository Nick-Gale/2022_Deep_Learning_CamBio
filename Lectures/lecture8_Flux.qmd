...
g = gradient(Params(W)) do
	f(x)
end
```

* Both of these methods of computation have advantages

* Do blocks are particularly useful for defining callback functions.

* Callbacks are useful when monitoring training progress (and this is most of what you will be doing in practice)
...
* Gradient descent is defined as: $W_{t+1} = W_t - \eta \frac{\partialL}{\partialW} $

* Gradient descent may be accelerated by momentum.

* Flux has support for multiple optimisers through an optimiser object class.

* These are given by ` opt = Descent(\eta), Momentum(\eta, \alpha), ADAM(\text{hyper-paramaters}), RMSProp, etc`

# Stochastic Gradient Descent

* Gradients can be approximated by a subset of data.

* Stochastic Gradient Dsecent is when these subsets are chosen randomly.
...
* Dropout layers freeze parameters.

* AlphaDropout layers freeze parameters and normalise mean and variance.

* There are extensive pooling layers: MaxPool((d1, d2)) is common.
...
* A generically complex model can be given by a function $f$. *rare*

* Flux can be told this is a model with `@functor f`.
...
* A convenience function `DataLoader` is provided to abstract a generator.

* It zips data pairs and can optionally shuffle with a given batch size.

```{julia}
 xvec = rand(40,100); yvec = rand(40);
data = DataLoader((xvec, yvec); batchsize=5, shuffle=true)
for (x, y) in data
	# loop eight times through batches of 5
end
```
# Loss Functions
...
* Flux makes this easy throught the `update!` API call.
...
* They are passed as optional keywords to train e.g. `train!(...; cb = ()->println("This callsback"))`
...
* For a GPU based model the data should be converted to CUDA arrays.
...
* Can have an indication of training time
